...
def drive(cfg, model_path=None, use_joystick=False, model_type=None,
    ...
    class Sensor:
        def __init__(self):
            ...
            self.payload=[0.0]
            self.wtg=0
            self.radio_number = bool(1)
            self.address = [b"1Node", b"2Node"]
        def run(self):
            if not self.radio.begin():
                raise RuntimeError("radio hardware is not responding")
            self.radio.setPALevel(RF24_PA_LOW)  # RF24_PA_MAX is default
            self.radio.openWritingPipe(self.address[self.radio_number])  # always uses pipe 0
            self.radio.openReadingPipe(1, self.address[not self.radio_number])  # using pipe 1
            self.radio.payloadSize = len(struct.pack("<f", self.payload[0]))
            self.radio.startListening()  # put radio in RX mode
            start_timer = time.monotonic()
            while (time.monotonic() - start_timer) < 2:
    ...
    V.add(Sensor(),  outputs=['wtg'], threaded=True)
    class Picmatch:
        def __init__(self):
            self.mylocation = 0
            self.template = []
            ...
            print('start!!!')
            self.template = cv2.imread('light_tem.jpg', 1)
            self.match=0

        def run_threaded(self, img):#线程模式下要把self.img设置为img
            self.img = img
            return self.mylocation,self.match

        def update(self):
            while True:
                self.mylocation,self.match = self.run(self.img)

        def run(self, img):#run部分不用设置self.img,可以直接用
            try:
                img = np.array(img)
            ...
            if (max_val1 >= 0.65):##self.time1是实例化的时间，匹配成功，且当且时间是实例化时间的2s之后，可以加1，把self.time1
                                    ##设置为当前时间，设置location，下一次如果匹配到，大于两秒说明是新图，小于两秒说明是就图，新图
                                    #位置加一，旧图不管，还是把self.time1设置为当前时间
                if (time.time() - self.time1 >= 1):
                    self.green_sign += 1
                    if (self.green_sign == 7):
                        self.green_sign = 1
                self.time1 = time.time()#第一次匹配到self.time1设置为第一次匹配到的时间第二次新匹配到的第二幅的时间间隔如果大于第一次的，就可以信号加一
                self.mylocation = self.green_sign

                print('where car is',self.mylocation)
                self.mylocation=int(self.mylocation)
                self.match=1
            else:
                self.match=0
    ...
    V.add(Picmatch(), inputs=['cam/image_array'], outputs=['wci','match'], threaded=True)
    ...
    class AfterPictureMatch:
        ...
        def run(self,wci,wtg,match):
            if wtg==0:
                self.mode=0
                self.extra=0
                return self.mode,self.extra
            else:
                print('wci',wci,'wtg',wtg)
                if wci==wtg:
                    self.mode=0
                    self.extra=1
                    return self.mode,self.extra
        ...
        def update(self):
            while True:
                self.mode,self.extra= self.run(self.wci,self.wtg,self.match)
        def run_threaded(self, wci,wtg,match):
            self.wci = wci
            self.wtg = wtg
            self.match=match
            return self.mode,self.extra
    ...
    V.add(AfterPictureMatch(),
          # inputs=['wim','wtg','gon'],
          inputs=['wci','wtg','match'],
          outputs=['mode','extra'], threaded=True)
    class Water:
        def __init__(self):
            self.extra=0
            self.makerobo = 24  # 选择18号GPIO引脚
            self.water_signal="close"
        ...
        def run(self,extra):
            ...
            if extra==1:
                ...
                GPIO.setup(self.makerobo, GPIO.OUT)  # 把18号口设置为输出口
                flag = 1
                i = 0
                for t in range(1):
                    GPIO.output(self.makerobo, flag)  # 在18号口输出电平
                    i = i + 1
                    time.sleep(3)
                    print("watering")
                    if flag == 0:
                ...
                self.water_signal="watering"
                return self.water_signal
            else:
                GPIO.setmode(GPIO.BCM)
                GPIO.setup(self.makerobo, GPIO.IN)
                
                
                return self.water_signal
        def update(self):
            while True:
                self.water_signal= self.run(self.extra)
        def run_threaded(self, extra):
            self.extra =extra
            return self.water_signal
    ...
    V.add(Water(),
          # inputs=['wim','wtg','gon'],
          inputs=['extra'],
          outputs=['water_signal'], threaded=True)
    ...
    class DriveMode:
        def run(self, mode,modes,
                    user_angle, user_throttle,
                    pilot_angle, pilot_throttle):

            if mode == 'user' or modes==0:
    ...
    V.add(DriveMode(),
          inputs=['user/mode','mode', 'user/angle', 'user/throttle',
